...
class CardsDeckScene extends SoftgamesScene {
    ...
    //array that store all cards
    public cards: CardSprite[] = [];

    //combined height that is calculated when cards loading in the scene
    public cardsHeight: number = 0;

    //animation width that is calculated at runtime based upon the available window width
    public cardsAnimationWidth: number = 200;

    //direction for animating cards left to right or right to left
    ...
    public update(timeDelta: number) {
        ...
        for (let i = 0; i < this.cards.length; i++) {
            ...
            if (card.startAnim < this.clock) {
                ...
                card.x =
                    (this.cardsAnimationWidth * 2 * EasingsFunc.CubicOut(animTime) -
        ...
        //flip the directions when all cards are animated
