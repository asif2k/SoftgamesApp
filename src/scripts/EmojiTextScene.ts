...
class TextImages extends PIXI.Container {
    ...
    public static TextsPool = new ObjectsPool(() => {
    ...
    public static ImagesPool = new ObjectsPool(() => {
    ...
    public setText(input: string) {
        ...
        if (tags) {
            ...
            parts.forEach(part => {
                ...
                if (part !== "") {
                    // check if part is a tag , that is some font settings or image to embed
                    // part = part.trim();
                    //if (rgTagsReplaced.test(part)) {
                    if (part.startsWith("{") && part.endsWith("/}")) {
                        //  console.log("pass", part);
...
export default class EmojiTextScene extends SoftgamesScene {
    ...
    public build(resources: Partial<Record<string, PIXI.LoaderResource>>) {
        ...
        const emojione = (resources.emojione?.texture as PIXI.Texture).baseTexture

        // load images from the texture which has 128x128 emoji icons
        TextImages.LoadImagesList(emojione, "m1", 128, 128)

        // can also pass custom tags
        TextImages.LoadImagesList(emojione, "", 128, 128, [":)", ";)"])

        //another different emojicons sprite sheet
        const emojione2 = (resources.emojione2?.texture as PIXI.Texture).baseTexture

        TextImages.LoadImagesList(emojione2, "m2", 20, 20)
        ...
        this.sampleWords = "This Lorem Ipsum alternative can be a great option for designs that deal with savory food or for any design team that loves their cheese It’s silly, fun and still gets the job done when it comes to the design process".split(" ")
    ...
    public update(timeDelta: number) {
        ...
        if (this.textImages) {
            ...
            const time = EasingsFunc.BounceOut(Math.sin(this.clock))
